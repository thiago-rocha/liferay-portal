...
form.fields = function(opt_data, opt_ignored) {
  var output = '';
  var fieldList63 = opt_data.fields;
  var fieldListLen63 = fieldList63.length;
  for (var fieldIndex63 = 0; fieldIndex63 < fieldListLen63; fieldIndex63++) {
    var fieldData63 = fieldList63[fieldIndex63];
    output += form.field(soy.$$augmentMap(opt_data, {field: fieldData63}));
  }
...
form.pages = function(opt_data, opt_ignored) {
  ...
  for (var pageIndex97 = 0; pageIndex97 < pageListLen97; pageIndex97++) {
    ...
    output += '<div style="background-color: #fff; border-radius: 4px; margin: 0 auto 24px auto; padding: 40px;"><h4 style="color: #9aa2a6; font-size: 21px; font-weight: 500; margin: 0;">' + soy.$$escapeHtml(pageData97.title) + '</h4>' + form.fields(soy.$$augmentMap(opt_data, {fields: pageData97.fields})) + '</div>';
  }
  return soydata.VERY_UNSAFE.ordainSanitizedHtml(output);
};
if (goog.DEBUG) {
