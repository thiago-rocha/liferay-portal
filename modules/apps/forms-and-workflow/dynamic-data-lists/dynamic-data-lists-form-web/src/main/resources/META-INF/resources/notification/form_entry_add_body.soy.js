...
form.fields = function(opt_data, opt_ignored) {
  var output = '';
  var fieldList43 = opt_data.fields;
  var fieldListLen43 = fieldList43.length;
  for (var fieldIndex43 = 0; fieldIndex43 < fieldListLen43; fieldIndex43++) {
    var fieldData43 = fieldList43[fieldIndex43];
    output += form.field(soy.$$augmentMap(opt_data, {field: fieldData43}));
  }
...
form.pages = function(opt_data, opt_ignored) {
  ...
  for (var pageIndex77 = 0; pageIndex77 < pageListLen77; pageIndex77++) {
    ...
    output += '<div style="background-color: #fff; border-radius: 4px; margin: 0 auto 24px auto; padding: 40px;"><h4 style="color: #9aa2a6; font-size: 21px; font-weight: 500; margin: 0;">' + soy.$$escapeHtml(pageData77.title) + '</h4>' + form.fields(soy.$$augmentMap(opt_data, {fields: pageData77.fields})) + '</div>';
  }
  return soydata.VERY_UNSAFE.ordainSanitizedHtml(output);
};
if (goog.DEBUG) {
