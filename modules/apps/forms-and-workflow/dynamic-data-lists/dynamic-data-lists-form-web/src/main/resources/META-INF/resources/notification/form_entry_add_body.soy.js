...
form.fields = function(opt_data, opt_ignored) {
  var output = '';
  var fieldList287 = opt_data.fields;
  var fieldListLen287 = fieldList287.length;
  for (var fieldIndex287 = 0; fieldIndex287 < fieldListLen287; fieldIndex287++) {
    var fieldData287 = fieldList287[fieldIndex287];
    output += form.field(soy.$$augmentMap(opt_data, {field: fieldData287}));
  }
...
form.pages = function(opt_data, opt_ignored) {
  var output = '';
  var pageList321 = opt_data.pages;
  var pageListLen321 = pageList321.length;
  for (var pageIndex321 = 0; pageIndex321 < pageListLen321; pageIndex321++) {
    var pageData321 = pageList321[pageIndex321];
    output += '<div style="background-color: #fff; border-radius: 4px; margin: 0 auto 24px auto; padding: 40px;"><h4 style="color: #9aa2a6; font-size: 21px; font-weight: 500; margin: 0;">' + soy.$$escapeHtml(pageData321.title) + '</h4>' + form.fields(soy.$$augmentMap(opt_data, {fields: pageData321.fields})) + '</div>';
  }
  return soydata.VERY_UNSAFE.ordainSanitizedHtml(output);
};
if (goog.DEBUG) {
