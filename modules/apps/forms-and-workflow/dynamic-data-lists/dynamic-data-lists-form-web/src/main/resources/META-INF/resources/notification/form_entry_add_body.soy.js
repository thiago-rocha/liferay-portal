...
form.fields = function(opt_data, opt_ignored) {
  var output = '';
  var fieldList142 = opt_data.fields;
  var fieldListLen142 = fieldList142.length;
  for (var fieldIndex142 = 0; fieldIndex142 < fieldListLen142; fieldIndex142++) {
    var fieldData142 = fieldList142[fieldIndex142];
    output += form.field(soy.$$augmentMap(opt_data, {field: fieldData142}));
  }
...
form.pages = function(opt_data, opt_ignored) {
  ...
  for (var pageIndex176 = 0; pageIndex176 < pageListLen176; pageIndex176++) {
    ...
    output += '<div style="background-color: #fff; border-radius: 4px; margin: 0 auto 24px auto; padding: 40px;"><h4 style="color: #9aa2a6; font-size: 21px; font-weight: 500; margin: 0;">' + soy.$$escapeHtml(pageData176.title) + '</h4>' + form.fields(soy.$$augmentMap(opt_data, {fields: pageData176.fields})) + '</div>';
  }
  return soydata.VERY_UNSAFE.ordainSanitizedHtml(output);
};
if (goog.DEBUG) {
