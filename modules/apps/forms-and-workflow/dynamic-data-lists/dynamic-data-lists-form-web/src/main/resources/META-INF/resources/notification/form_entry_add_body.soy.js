...
form.fields = function(opt_data, opt_ignored) {
  var output = '';
  var fieldList74 = opt_data.fields;
  var fieldListLen74 = fieldList74.length;
  for (var fieldIndex74 = 0; fieldIndex74 < fieldListLen74; fieldIndex74++) {
    var fieldData74 = fieldList74[fieldIndex74];
    output += form.field(soy.$$augmentMap(opt_data, {field: fieldData74}));
  }
...
form.pages = function(opt_data, opt_ignored) {
  ...
  for (var pageIndex108 = 0; pageIndex108 < pageListLen108; pageIndex108++) {
    ...
    output += '<div style="background-color: #fff; border-radius: 4px; margin: 0 auto 24px auto; padding: 40px;"><h4 style="color: #9aa2a6; font-size: 21px; font-weight: 500; margin: 0;">' + soy.$$escapeHtml(pageData108.title) + '</h4>' + form.fields(soy.$$augmentMap(opt_data, {fields: pageData108.fields})) + '</div>';
  }
  return soydata.VERY_UNSAFE.ordainSanitizedHtml(output);
};
if (goog.DEBUG) {
